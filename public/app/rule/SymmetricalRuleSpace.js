...
export function createRandomSymTable(stateCount) {
    const symStateCount = 1 + symmetryMap[stateCount - 1][stateCount - 1];
    const sizePower = (stateCount ** 2) * symStateCount;
    return Array.from({ length: sizePower }, () => Math.floor(Math.random() * stateCount));
}
export function createEmptySymTable(stateCount) {
    const symStateCount = 1 + symmetryMap[stateCount - 1][stateCount - 1];
    const sizePower = (stateCount ** 2) * symStateCount;
    return Array.from({ length: sizePower });
}
...
export function symToFull(symTable, stateCount) {
    const symStateCount = 1 + symmetryMap[stateCount - 1][stateCount - 1];
...
export function getSymRuleSpaceSize(stateCount) {
    const symStateCount = 1 + symmetryMap[stateCount - 1][stateCount - 1];
    const sizePower = (stateCount ** 2) * symStateCount;
...
export class SymmetricalRuleSpace {
    constructor(stateCount) {
        ...
        this.symStateCount = 1 + symmetryMap[this.stateCount - 1][this.stateCount - 1];
    ...
    generateRandomRule() {
        ...
        return new (class extends Rule {
            constructor() {
                super(...arguments);
                this.ruleSpaceCode = getNumberFromDigits(symTable, this.stateCount);
            }
        })(stateCount, table);
    }
}
